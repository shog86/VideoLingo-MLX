...
def download_video_ytdlp(url, save_path='output', resolution='1080', progress_callback=None):
    ...
    # Progress hook for streaming download status to UI
    if progress_callback:
        def _yt_dlp_hook(d):
            if d.get('status') == 'downloading':
                ...
                progress_callback(percent=pct, speed=speed, eta=eta,
                                  downloaded=downloaded, total=total)
            elif d.get('status') == 'finished':
                progress_callback(percent=100, speed=None, eta=0,
                                  downloaded=0, total=0, finished=True)
        ydl_opts['progress_hooks'] = [_yt_dlp_hook]
...
def find_audio_files(save_path='output'):
    ...
    # Exclude dubbing outputs so they are never mistaken for user input
    audio_files = [file for file in audio_files if os.path.basename(file) not in GENERATED_AUDIO_NAMES and not file.startswith("output/output")]
